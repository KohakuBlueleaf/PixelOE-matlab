...
weight = 1-weight;
thickness2 = outline_expansion(img, 2, 2, patch_size, 6, 3);

% crop_box = [1, size(weight, 2)-128, 128, 128];
crop_box = [1, 1, 128*8, 128*8];

edge = outline_expansion(img, 2, 2, patch_size, 9, 4)-img;
edge = (edge+1)./2;
...
subplot(2, 3, 3); imshow(imcrop(thickness2, crop_box)); title('Thickness 2'); %imshow(edge); title('Thickness2 edge');
